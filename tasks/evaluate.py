...
import numpy as np
from tasks.pytorch_utils import forward_audio, forward_video, forward_av
...
class Evaluator(object):
    ...
    def evaluate_audio(self, data_loader):
        ...
        try:
            ...
            auc = np.array([0.5] * target.shape[1])
        ...
        precision = precision_score(target_acc, clipwise_output_acc, average=None)
        recall = recall_score(target_acc, clipwise_output_acc, average=None)
        f1 = f1_score(target_acc, clipwise_output_acc, average=None)
    ...
    def evaluate_av(self, data_loader):
        ...
        output_dict = forward_av(
            model=self.model, generator=data_loader,
            return_input = False, return_target=True)
        ...
        precision = precision_score(target_acc, clipwise_output_acc, average=None)
        recall = recall_score(target_acc, clipwise_output_acc, average=None)
        f1_socre = f1_score(target_acc, clipwise_output_acc, average=None)
        cm = confusion_matrix(target_acc, clipwise_output_acc)
        # cm_display = ConfusionMatrixDisplay(cm).plot()
        ...
        message = '\n' + message
        statistics = {'average_precision': average_precision, 'accuracy': acc, 'auc': auc, 'message': message,
                      'confu_matrix': cm, 'precision':precision, 'recall':recall, 'f1':f1_socre}
