...
if __name__ == '__main__':
    ...
    width = 512
    input_features= 204
    input_data = torch.randn(batch_size, 1, input_features, height, width)

    # Convert input data to a tensor
    input_tensor = torch.Tensor(input_data)
    model = CNN_1D(input_features=input_features, n_classes=7)  # Instantiate the model with the appropriate parameters
