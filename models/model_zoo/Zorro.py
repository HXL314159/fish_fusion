...
class Zorro(nn.Module):
    ...
    def forward(
        ...
        assert all([divisible_by(numer, denom) for denom, numer in zip(self.video_patch_size, tuple(
            video.shape[-3:]))]), f'video shape {video.shape[-3:]} needs to be divisible by {self.video_patch_size}'
        ...
        audio_tokens, fusion_tokens, video_tokens = map(lambda t: rearrange(t, 'b ... d -> b (...) d'),
                                                        (audio_tokens, fusion_tokens, video_tokens))
