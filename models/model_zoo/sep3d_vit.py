...
class DSSA(nn.Module):
    ...
    def forward(self, x):
        ...
        batch, height, width, heads, wsz, frames, frame_patch_size = x.shape[0], *x.shape[-2:], self.heads, self.window_size, self.frames, self.frame_patch_size
        assert (height % wsz) == 0 and (width % wsz) == 0, f'height {height} and width {width} must be divisible by window size {wsz}'
        num_windows = (height // wsz) * (width // wsz) * (frames // frame_patch_size)
    ...
    def __init__(
        ...
        for ind in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, DSSA(dim, heads = heads, dim_head = dim_head, dropout = dropout, frames=frames, frame_patch_size=frame_patch_size)),
        ...
        *,
        ...
        for ind, ((layer_dim_in, layer_dim), layer_depth, layer_stride, layer_heads, layer_window_size) in enumerate(zip(dim_pairs, depth, strides, *hyperparams_per_stage)):
            ...
            self.layers.append(nn.ModuleList([
                ...
                PEG(layer_dim),
                Transformer(dim = layer_dim, depth = layer_depth, heads = layer_heads, ff_mult = ff_mult, dropout = dropout,frames=10, frame_patch_size=2, norm_output = not is_last),
            ]))
