...
class ConvMixer(nn.Module):
    def __init__(self, dim=32, depth=6, kernel_size=7, patch_size=2, n_classes=128):
        super().__init__()
...
if __name__ == '__main__':
    model = ConvMixer(dim=256, depth=10).to(device)
    input = torch.randn(5, 3, 64, 64).to(device)
    out = model(input)
    print(out.shape)
