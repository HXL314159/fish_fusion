...
class I3D(nn.Module):
    def __init__(self, classes_num=4, dropout_drop_prob=0.5, input_channel=3, spatial_squeeze=True):
        ...
        self.features = nn.Sequential(
            ...
            Mixed_5b(), # (832, 8, 7, 7)
            Mixed_5c(), # (1024, 8, 7, 7)
            nn.AvgPool3d(kernel_size=(1, 6, 6), stride=1),# (1024, 8, 1, 1)
            nn.Dropout3d(dropout_drop_prob),
            nn.Conv3d(1024, classes_num, kernel_size=1, stride=1, bias=True),# (400, 8, 1, 1)
        )
...
if __name__ == '__main__':
    model = I3D().cuda()
    input = torch.randn(1, 3, 50, 224, 224).cuda()
    output = model(input)
