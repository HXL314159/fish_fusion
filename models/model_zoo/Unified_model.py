...
#         x = self.to_latent(x)
#         if modality == 'a':
#             output = self.mlp_head_a(x)
#         elif modality == 'v':
#             output = self.mlp_head_v(x)
#         elif modality == 'av':
#             output = self.mlp_head_av(x)
#         return output
...
class Unified_model(nn.Module):
    def __init__(self, *, image_size, image_patch_size, frames, frame_patch_size,
        ...
        num_patches_av = 2 + num_patches_v

        self.pos_embedding_v = nn.Parameter(torch.randn(1, num_patches_v + 1, dim))
        self.pos_embedding_a = nn.Parameter(torch.randn(1, 2 + 1, dim))
        self.pos_embedding_av = nn.Parameter(torch.randn(1, num_patches_av + 1, dim))
...
if __name__ == '__main__':
    ...
    video = torch.randn(1, 3, 8, 224, 224).cuda()
    audio = torch.randn(1, 1, 128, 128).cuda()
    # output = model(audio, None, 'a')
    output1 = model(audio, video, 'av')
    from thop import profile
    from thop import clever_format
    macs, params = profile(model, inputs=(audio, video, 'av'))
