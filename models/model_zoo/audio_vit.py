...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.qkv_proj = nn.Linear(embed_dim, 1 + (2 * embed_dim), bias=False)
    ...
    def __init__(self, dim, depth, mlp_dim, dropout = 0.):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, Attention(embed_dim=dim,
                                        attn_dropout=0.1,
                                        inplace=False,
                                        bias=True)),
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, mlp_dim, pool='cls', channels=3, dropout=0., emb_dropout=0.):
        super().__init__()
        ...
        self.transformer = Transformer(dim, depth, mlp_dim, dropout)
...
if __name__ == '__main__':
    ...
    # audio = torch.randn(2, 3, 224, 224).cuda()
    # output = model(audio)
    # print(output.shape)
