...
class Cnn6(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv_block1(x, pool_size=(2, 2), pool_type='avg')  # (100, 64, 125, 32)
        x = F.dropout(x, p=0.2, training=self.training)
        x = self.conv_block2(x, pool_size=(2, 2), pool_type='avg')  # (100, 128, 62, 16)
        x = F.dropout(x, p=0.2, training=self.training)
        x = self.conv_block3(x, pool_size=(2, 2), pool_type='avg')  # (100, 256, 31, 8)
        x = F.dropout(x, p=0.2, training=self.training)
        x = self.conv_block4(x, pool_size=(2, 2), pool_type='avg')  # (100, 512, 15, 4)
        x = F.dropout(x, p=0.2, training=self.training)
        x = torch.mean(x, dim=3)  # (100, 512, 15)
        # x1 (100, 512)
        (x1, _) = torch.max(x, dim=2)
        # x2: (100, 512)
        x2 = torch.mean(x, dim=2)
...
class ResNet18(nn.Module):
    def __init__(self, classes_num):
        ...
        self.conv_block_after1 = ConvBlock5x5(in_channels=512, out_channels=2048)
...
if __name__ == '__main__':
    # model = ResNet38(classes_num=4)

    # model = resnet18()
    # input = torch.randn(10, 1, 126, 64)
    # out = model(input)
