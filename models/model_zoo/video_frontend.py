...
class Conv3dResNet(torch.nn.Module):
    ...
    def forward(self, xs_pad):
        ...
        xs_pad = self.frontend3D(xs_pad) # 4, 64, 10, 49, 49
        Tnew = xs_pad.shape[2]    # output should be B x C2 x Tnew x H x W (Tnew = 10)
        xs_pad = threeD_to_2D_tensor(xs_pad)  # 40, 64, 49, 49
        xs_pad = self.trunk(xs_pad)  # 40, 512
        xs_pad = xs_pad.view(B, Tnew, xs_pad.size(1))    # 4, 10, 512
...
if __name__ == '__main__':
    model = Conv3dResNet()
    # model = ResNet()
    input = torch.randn(4, 1, 126, 64)
    # input = torch.randn(4, 10, 3, 196, 196)  # Batch, Time, H, W
